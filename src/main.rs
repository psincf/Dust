...
fn main() {
    ...
    let mut world = Box::new(World {
        ...
        benchmark_draw: 0.0,
        threadpool: threadpool::Threadpool::new_with_threads(8),
    });
    world.reset();
                            ...
                            ElementState::Pressed => {
                                match input.virtual_keycode {
                                    Some(VirtualKeyCode::C) => {
                                        match world.compute_mode {
                                            ComputeMode::CPU => { world.compute_mode = ComputeMode::GPU },
                                            ComputeMode::GPU => { world.compute_mode = ComputeMode::CPU }
                                        }
                                    }
